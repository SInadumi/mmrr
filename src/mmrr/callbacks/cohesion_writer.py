...
from mmrr.writer.cohesion import ProbabilityJsonWriter
...
class CohesionWriter(BasePredictionWriter):
    ...
    def write_on_epoch_end(
        ...
        knp_writer = PredictionKNPWriter(
            dataset, self.flip_writer_reader_according_to_type_id
        )
        ...
        for pred in predictions:
            ...
            for example_id, relation_logits, source_mask_logits in zip(
                ...
                eid_to_relation_prediction[example_id.item()] = (
        ...
        _ = json_writer.write(
            eid_to_relation_prediction, destination=self.json_destination
        )
    ...
    def write_document(self, document: Document) -> None:
        if isinstance(self.knp_destination, Path):
            ...
            self.knp_destination.joinpath(f"{document.doc_id}.knp").write_text(
                document.to_knp()
            )
